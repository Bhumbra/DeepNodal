...
#-------------------------------------------------------------------------------
  def set_ipverge(self, ipv = None, *ipv_args, **ipv_kwds):
    """
    ipv is a vergence specification that unites inputs so that their numbers must
    ...
    elif type(self.ipv) is list:
      if not(len(self.ipv)):
        self.ipv = None
        return
      elif type(self.ipv[0]) is not list:
        self.ipv = [self.ipv]
  ...
  def set_opverge(self, opv = None, *opv_args, **opv_kwds):
    ...
    if type(self.opv) is list:
      if not(len(self.opv)):
        self.opv = None
        return
      elif type(self.opv[0]) is not list:
        self.opv = [self.opv]
    else:
